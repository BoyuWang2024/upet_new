...
- ConfidenceHead 分支只永久保存通用的零权重配置能力、单元测试和设计文档。
- 通用代码通过 GitHub 推送，并由 bywang 服务器在同一 ConfidenceHead 分支拉取。
- 九份服务器专用 YAML 和九份 Slurm 脚本仅作为本地临时部署资产，不提交到 Git。
- 部署验证后删除本地临时副本，恢复干净的本地发布工作区。
- 服务器上的九份 YAML 和九份脚本作为未跟踪运行文件保留。
- 服务器现有的已修改 full_gpu.yaml 和未跟踪 run/submit.sh 不覆盖、不提交。
