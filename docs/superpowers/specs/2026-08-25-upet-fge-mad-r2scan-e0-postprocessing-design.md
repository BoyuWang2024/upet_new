...
- direct E0 只使用 MAD R2SCAN val 中的 raw energy 与 atomization energy 恢复目标 E0。
- model-aware E0 只使用 MAD R2SCAN val 的标签以及 K=8 ensemble mean 拟合。
- 同一套 model-aware 修正应用到全部 8 个成员；禁止逐成员独立拟合。
- test 标签仅在校准产物封存后用于最终评估，不参与拟合、参数选择或返工。
...
- `energy - atomization_energy` 的逐元素重构预审计 RMSE 约 `5.3e-9 eV`，
  最大绝对误差约 `1.6e-7 eV`。
...
仅在过滤后的 val 上求 MAD 原子参考能量：

\[
X_{val} E0_{MAD} = E_{raw,val}^{MAD} - E_{atomization,val}^{MAD}.
\]

求解使用 float64、固定的 89 元素列顺序、无截距、无正则的 SVD 最小范数解。虽然该数据
在预审计中近乎精确可重构，仍必须保存 singular values、rank、condition number、RMSE 和
最大绝对重构误差，禁止静默接受欠秩或非有限结果。

对 val 和 test 的每个成员应用：

\[
E_{direct}^{(k)} = E_{original}^{(k)} - X E0_{MATPES} + X E0_{MAD}.
\]
...
- MAD E0 的 val 重构要求 RMSE 不超过 `1e-6 eV/structure`、最大绝对误差不超过
  `5e-6 eV/structure`；
...
- `fge/energy_correction.py`：纯 float64 校准、应用、恒等式验证和诊断数据结构；
- `scripts/correct_energy.py`：分成 `calibrate` 与 `apply` 两个显式阶段的薄 CLI；
...
`reference_float64/{val,test}` 分别绑定从 immutable extxyz 重读的 raw energy、
atomization energy、结构顺序和数据 SHA。`shared_force/manifest.json` 只指向并验证原
prediction 中唯一一套 Force 数据，
不为三种 Energy 方法复制三遍 Force。

`calibration_manifest.json` 必须记录：
...
- 固定元素顺序、MATPES E0、MAD E0、model-aware delta；
...
chunk identities、封存后的 calibration manifest SHA，以及三种 Energy 和共享 Force 的所有
产物身份。任何身份不匹配都应硬失败，不能按数组长度或当前位置猜测对应关系。

## 8. 无数据泄漏执行顺序

1. 验证 K=8 成员、checkpoint 与 CompositionModel 一致性。
2. 复现 val 支持元素过滤并重读 float64 raw/atomization reference。
3. 验证或完成一次 val predict；不触碰 test 标签。
4. 在 val 上分别求 `E0_MAD` 和公共 `delta`，生成并封存 calibration manifest。
5. 重新读取 calibration manifest，验证其 SHA；此后校准参数不可修改。
6. 只读验证完整 test prediction，按结构 ID/chunk identity 关联 test 的 float64 reference。
7. 同时生成 baseline、direct E0、model-aware E0 sidecar 和一份共享 Force manifest。
8. 计算 test 指标并绘图；无论结果优劣，都不得返回第 4 步重调参数。
9. 对原 prediction 树再次做 SHA/size/mtime 审计，证明前后未改变。

`calibrate` 阶段的接口不得接受 test 路径；`apply` 阶段只接受已经封存且 SHA 明确的校准产物。
这一接口隔离是防止 test 泄漏的主要机制，而不仅是一条操作约定。
...
- val 组成矩阵 rank 小于 89，或出现非有限 SVD/校准结果；
- test 出现 val 未覆盖的模型支持元素，或 E0/元素映射不完整；
- 任一 FGE 成员修改了 CompositionModel，或成员/manifest 身份不一致；
- val/test prediction manifest 不完整，结构数、顺序、ID 或 chunk identity 不匹配；
- 原始/过滤数据 SHA、metadata SHA 或封存后的 calibration SHA 漂移；
- float64 reference 无法按结构一一关联，或 raw/atomization energy 缺失、重复、非有限；
...
- 用已知逐元素 E0 的满秩合成数据验证 direct E0 精确恢复和元素顺序；
...
- 验证 direct 恒等式和 test 元素被 val 覆盖；
- 验证 `calibrate` 不接受 test 输入、`apply` 不接受未封存 calibration。
...
2. val 过滤结果为 16,098 structures / 310,432 atoms，组成矩阵 rank=89；
3. MAD E0 重构、direct 恒等式和 model-aware SVD 诊断均在预先固定的 float64 容差内；
4. 三种 Energy sidecar、共享 Force manifest、result manifest 与 4 组 PNG/PDF 全部完整；
5. 三种 Energy 使用同一原始 population STD，Force 数据未改变；
6. test 标签未进入 calibration，且没有依据 test 结果重调任何参数；
