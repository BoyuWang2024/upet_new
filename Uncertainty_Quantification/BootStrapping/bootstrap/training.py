...
def fit_runtime(
    ...
    *,
    ...
    on_epoch: Callable[[TrainingResume], None] | None = None,
) -> TrainingResult:
    ...
    if resume is not None:
        ...
        if resume.runtime_state:
            if not callable(restore):
                raise HardFailure("runtime cannot restore its persisted state")
            restore(copy.deepcopy(resume.runtime_state))

    def make_resume(completed_epoch: int) -> TrainingResume:
        capture = getattr(runtime, "capture_state", None)
        runtime_state = {} if not callable(capture) else capture()
        if not isinstance(runtime_state, dict):
            raise HardFailure("runtime capture_state must return a mapping")
        return TrainingResume(
            completed_epoch=completed_epoch,
            raw_state=_clone_state(runtime.model.state_dict()),
            ema_state=_clone_state(ema.shadow),
    ...
    resume_state = make_resume(max_epochs)
